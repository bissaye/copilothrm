import React, { useEffect, useState} from "react";
import { avatars} from "../../assets/images";
import { DefaultButton } from "../components/ui";
import { faArrowLeft, faArrowRight, faPlus } from "@fortawesome/free-solid-svg-icons";
import { useAuthStore, useSpinnerStore } from "../../services/store";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { useNavigateById } from "../../hooks";
import { pageIds } from "../../utils/constantes";
import { useIntl } from "react-intl";
import { UserData } from "../../services/api/DTO/response";
import { useApiServices } from "../../services/api/ApiServiceContext";
import { useStaffUseCase } from "../../services/api/usescases";
import { StaffOrganisationContent } from "../../services/api/DTO/response/staff";
import { toastify } from "../../utils/toasts";

export const ChooseOrg: React.FC = () => {
    const {formatMessage} = useIntl();
    const {signOut} = useAuthStore();
    const NavigateById = useNavigateById();
    const { staffService } = useApiServices()
    const { getUserOrganisations } = useStaffUseCase(staffService)
    const {showSpinner, hideSpinner} = useSpinnerStore()
    const [userOrganisations, setUserOrganisations] = useState<StaffOrganisationContent[] | null>(null)

    const user: UserData = JSON.parse(localStorage.getItem("user")!)

    const loggedUser = user.staff.user;

    const handleOpenOrg = (orgId: string) => {
        const currentOrg = userOrganisations ? userOrganisations.find(org => org.organisation.organisationId == orgId) : null;
        localStorage.setItem('currentOrg', JSON.stringify(currentOrg));
        NavigateById(pageIds.HomePage)
    }

    useEffect(() => {
        async function getOrganisations() {
            try {
                showSpinner()
                await getUserOrganisations().then((response) => {
                    setUserOrganisations(response.content)
                    hideSpinner()
                })
            }
            catch(error: any){
                hideSpinner()
                toastify('error', error.message)
            }
        }

        getOrganisations()
    }, [])

    return <div className="flex flex-col justify-start items-center w-full h-full py-7 gap-5">
            {/* <img src={imagesLogo.main}/> */}
            <div className="">
                <h1 className=" font-heading font-bold text-t8 text-center">
                    {formatMessage({id:"welcome_back"})}
                </h1>
                <p className="font-body text-t6 text-center my-3">
                    {formatMessage({id:"choose_a_workspace_below"})} <br/>
                    {formatMessage({id:"to_work_with_your_team_again"})}
                </p>
            </div>
            
            <div className="lg:w-[676px] min-w-[350px] flex flex-col justify-start items-start rounded-m border border-gray-250">
                <div className=" bg-gray-400 w-full">
                    <p className=" px-4 py-3 font-heading text-t3">
                        <span className="text-gray-800">{formatMessage({id:"organization_for"})}</span>
                        <span className="text-black font-bold">&nbsp;{loggedUser.username}</span>
                    </p>
                </div>
                <div className=" max-h-56 w-full flex flex-col justify-start items-start overflow-x-hidden overflow-y-scroll">
                    {
                        userOrganisations ? 
                        userOrganisations?.map((organisation, index) => (
                            <div className=" h-28 w-full pl-2 lg:pr-12 py-3 flex flex-row justify-between items-center border border-gray-250" key={`org-table-row-${index}`}>
                                <div className=" h-full flex flex-row justify-between items-center gap-2 ">
                                    <img alt="" className=" w-s16 h-s16 rounded-xl shadow-l" src={"data:image/jpeg;base64,"} />
                                    <div className="h-full flex flex-col justify-around items-start">
                                        <h1 className="font-body font-bold text-black text-t3">
                                            {organisation.organisation.raisonSociale}
                                        </h1>
                                        <div className="flex">
                                            <img alt="" className="rounded-full w-5 h-5" src={avatars.avatarLandingPage}/>
                                            <img alt="" className="rounded-full w-5 h-5" src={avatars.avatarLandingPage}/>
                                            <img alt="" className="rounded-full w-5 h-5" src={avatars.avatarLandingPage}/>
                                            <span className="text-gray-800 text-t2 font-body">&nbsp;&nbsp;{organisation.organisation.nbreCollaborateurs} {formatMessage({id:"members"})}</span>
                                        </div>
                                    </div>
                                </div>
                                <FontAwesomeIcon icon={faArrowRight} onClick={()=>{handleOpenOrg(organisation.organisation.organisationId)}} className=" border border-slate-600 p-1 w-4 rounded-full cursor-pointer hover:text-primary hover:border-primary"/>
                            </div>
                        ))
                        :
                        <div className=" h-28 w-full pl-2 lg:pr-12 py-3 flex flex-row justify-between items-center border border-gray-250">
                            {formatMessage({id:"no_organisations"})}
                        </div>
                    }
                </div>
            </div>

            <div className="flex flex-col md:flex-row gap-4">

                <DefaultButton
                    type="primary"
                    bgWhite={false}
                    text={formatMessage({id:"add_an_organization"})}
                    width={237}
                    icon={faPlus}
                    onClick={() => NavigateById(pageIds.AddOrganisation)}
                />

                <DefaultButton
                    type="secondary"
                    bgWhite={false}
                    text={formatMessage({id:"sign_out"})}
                    className='hover:bg-secondary-400 transition duration-300 ease-in-out'
                    width={237}
                    icon={faArrowLeft}
                    onClick={() => {
                        signOut()
                    }}
                />
            </div>
        </div>
}